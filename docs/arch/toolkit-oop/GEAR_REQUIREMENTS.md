...
#[toolkit::gear(consumes = [SomeApi])]
pub struct MyGear { ... }

// src/api/rest/handlers.rs
let client = services.some_api.resolve().await?;
```
...
#[derive(Parser)]
#[command(name = "my-gear-oop")]
#[command(about = "my-gear OoP gear")]
...
| Context | Plane | Carrier | Use for |
|---|---|---|---|
| `PlatformSecurityContext` | Platform | `X-ToolKit-Internal-Token` (SA token in Profile 3) | DirectoryService registration, GTS registration, heartbeats. |
...
| Context | Plane | Carrier | Use for |
|---|---|---|---|
| `SecurityContext` | Tenant | `Authorization: Bearer <jwt>` | User requests, gear-to-gear tenant calls, S2S tenant-scoped jobs. |
...
pub trait MyApiRest: MyApi {
    #[toolkit::rest(method = POST, path = "/my-gear/v1/do-thing")]
    async fn do_thing(&self, req: DoThingRequest) -> DoThingResponse;
}
```

```rust
// Consumer gear's registered_gears.rs
#[toolkit::gear(consumes = [MyApi])]
