...
5. **Plugin depth is decided per direction.** The read path is a plugin surface in v1 (point 4). A `GraphStoreV1` write-side plugin is a v2 candidate, and this ADR records what it must carry so the decision is not relitigated from scratch: the Concurrency and Ordering Contract is currently expressed in PostgreSQL transactions — the idempotency record committed atomically with the batch, an exclusive lock on the scope-registry row held through commit, generation fencing compared-and-set under that lock, `ON DELETE RESTRICT` on edge endpoints, and one snapshot shared by every search arm of a request. A store plugin satisfies all of it or the guarantees quietly weaken, and a "pass the query through" contract cannot express any of it. The cost is also wider than "writes": search runs tsvector, pgvector KNN and RRF fusion in the same statements, so a store plugin is the whole data plane. v1 therefore pays one forward-compatibility cost instead — no PostgreSQL concept appears in the REST or SDK contract — so the split stays possible without an API break.
